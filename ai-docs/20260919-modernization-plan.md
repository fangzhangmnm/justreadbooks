...
- 2026-09-19（深夜 2）：**0.1.2**：① 沉浸阅读（user「阅读的时候能不能隐藏顶栏…同意，我也不喜欢左右翻章」）：阅读时顶栏藏、中间 1/3 轻点开关、滚动即收、左右 1/3 无动作；② user 真机报「点了 gallery 之后有卡顿」→ 根因 = 切屏 / 切书路径 `await` 了云端往返（valuable-save commit）——**违反前提 0 的「永不为网络转圈」，教训：任何 UI 路径只准等 IDB，云端推送一律 `void` 丢后台**；冒烟加计时门（书架 < 400ms）。29 项 smoke 绿。
- 2026-09-19（深夜 3）：**0.1.3**（user「缺口都做，pdf 我倾向于开新的 agent」）：gallery **0.3.1** `onRenamed` hook → 书架改名/移动不再丢阅读位置与切章规则；naming 边界剥 `.txt`（改名框不带扩展名、不可能把书改成非 txt）；开不了的书清 last-open 指针、出「继续读」；目录按层级缩进；CLAUDE.md 持久层表补齐；PDF 给新 agent 的 handoff = `20260919-pdf-viewer-handoff.md`。31 项 smoke 绿（+ 书架改名非活动书 → 位置跟着搬）。
- 2026-09-19（深夜 4）：**切分审计**（user「onRenamed 这个不应该是 gallery 的，而是应该是 store 的！gallery 是前端…查一下还有什么切分错误的」「同意，然后继续查…think critically」）→ **store 0.14.0**（`files.onRenamed` 身份变更事件；云腿在线 = `isOnline ∧ signedIn`，修改名慢 1.2s；`config.hiddenName`）、**gallery 0.3.2**（撤 `hide` / `onRenamed`）、**JRB 0.1.4**（订阅 store 事件；`hiddenName` 进 createStore；删死代码 `watchBooks`）。**审下来留在原处的**（理由）：`tile.subtitle/marker/layout` = 纯呈现；`keepOffline` 动词 = UI 发起 store 面；`galleryItemFromStoreItem` 把 9 态 syncState 映射成布尔（纯映射非再推导，但正是 store 注释警告的「越狱」形状——将来 gallery 应直接吃 syncState）；`book-prefs.header` = app 派生数据存 synced collection（换取书架不读字节）；`valuable-save` 写盘节律留 app（trivial 判定是域逻辑，JRP 同款）；`uploadBook` 撞名加后缀 = app 命名政策（store 有 mode:"new" 撞名抛做兜底）；反向（store 做了前端的事）：busy/冲突 sheet/逃生闸都是「决策点回调」不是 UI 本体，符合设计。**未做**：`restoreTrash` 恢复到别名也是身份变更、目前不发 onRenamed（边缘）；`open({onProgress})` 是 store 缺的面（JRB 用 keepOffline 先囤再 open 绕了一下，属正当组合不是越狱）。
- 2026-09-19（深夜 5）：**0.1.5**（user「galleryItemFromStoreItem 把 store 的 9 态 syncState 映射成一堆布尔……将来 gallery 该直接吃 syncState。做」）→ **gallery 0.4.0**：`GItem = {name, syncState, size?, lastModified?}`，徽章 `Record<SyncState, BadgeKind>` 直映，读状态只经 `hasLocalCopy / hasCloudCopy / cloudBytesNewer` 三谓词，登出压扁交回 store 的 ListContext（153 测绿）；JRB `sizeOf` 改读 `item.size`。同批：user「强制版本更新的那个按钮是不是没有做？」→ 已有（设置 → 应用 → 清缓存重启，无头验证 toast「已清缓存重启 · v」），补一个 **「检查更新」**（`shell.checkForUpdate()` = poke `registration.update()` 等新 SW 装完 → 「有新版本」+ 刷新 toast / 「已是最新 · v」/ 「此环境无法检查」；冒烟 32/32）。**restoreTrash 不发 onRenamed**（user「从数据模型上我觉得这个不是 rename」）：live 命名空间三事件 created / renamed / deleted，恢复 = created，(2) 只是新名字的选择；伴生数据按路径键、删后即孤儿；固有 wart = 删 A 再上传不同的同名 A 继承旧位置（接受）。store 记录 doc 已改。edited by Claude Fable 5.1
