...
### 4.5. (历史 bug,已修)旧设备覆盖云端新进度 — 双根因

User 报:"旧设备上的阅读记录被错误覆盖了"。复现场景:

> A 设备读到 ch:50,断网 2 天。B 设备读到 ch:200 (云端最新)。A 重连开 app:
> jumpscare 显示 backup 的 ch:50。user 没意识到云端有 ch:200,随手 scroll
> 看了看。1s 后 reconcile 拉远端 → 但本地 setPosition 刚打了 NOW 时间戳 →
> merge 反过来选了 ch:50 → 云端 ch:200 被本地 ch:51 覆盖。

**两个 bug 叠加导致**:

- **Bug A**: `flush()` 在 `knownETag === null` 时(initSession Graph 读失败 / 还没读)
  调 `writeApprootJson(path, data, null)` —— 后者用 `conflictBehavior=replace` + **无
  If-Match** → 服务端无条件覆盖。修法:flush 看到 null knownETag → 先 `readApprootJson`
  拿 eTag + `mergeByTimestamp` → 再 push。`checkRemoteChanged` 也改成 null 返 true
  (本来 null = "不知道云端有啥",应该当作"有变化"去拉一次)。
- **Bug B**: 启动后 user scroll 触发 `setPosition` 打 `positionAt = NOW`,merge 时这个
  NOW 比 cloud 的 "T_recent (但 < NOW)" 大 → 反向覆盖。修法:`reconcilePending` flag,
  boot 后 reconcile 完之前 setPosition 不写 session(只更新 viewer 状态)。reconcile 完
  flag 自然 clear,继续正常 setPosition。

Bug B 的 trade-off:reconcile 期间(1-2 秒)user scroll 不被持久化。可以接受 ——
viewer scroll 由 500ms debounce 触发,reconcile 完用户继续看新位置自然会触发新的
setPosition。极端情况下用户 scroll 完正好 reconcile 完那一刻"停下"才会丢那一格 →
他下一次小动作就保存了。
